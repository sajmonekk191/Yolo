    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block w-full max-w-md p-6 my-8 text-left align-middle transition-all transform bg-white shadow-xl rounded-xl">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-900">Upravit rozpočet</h3>
...
const handleSubmit = async () => {
  ...
  // Kontrola duplicity (kromě aktuálně upravovaného rozpočtu)
  const existingBudget = financeStore.budgets.find(b => 
    b.id !== props.budget.id &&
  ...
  try {
    const result = await financeStore.updateBudget(props.budget.id, {
    ...
    console.error('Chyba při aktualizaci rozpočtu:', err)
  } finally {
...
onMounted(() => {
  // Naplň formulář daty z rozpočtu
  form.category_id = props.budget.category_id
  form.amount = props.budget.amount
  form.month = props.budget.month
  form.year = props.budget.year
  form.is_active = props.budget.is_active
})
</script>